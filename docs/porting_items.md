...
-   **Task H.2:** Refactor `Game` class to manage a `current_state` and delegate main loop calls. (Status: Complete)
...
-   **Task H.2.6** Relocate `toggle_*` functions to appropriate states or a dedicated view manager. (Status: In Progress)
    -   **Task H.2.6.A:** Create a `ViewManager` class to encapsulate the `_toggle_view` logic and the `_modal_stack`. (Status: Complete)
...
-   **Task H.2.6.E:** Migrate `toggle_home_view` logic from `Game` to `HomeState`. (Status: Complete)
    -   **Task H.2.6.F:** Refactor remaining `toggle_*_view` calls in `Game` to use the `ViewManager`. (Status: Complete)
    -   **Task H.2.6.G:** Remove redundant `toggle_*_view` methods from `Game` once their logic has been fully migrated. (Status: In Progress)
-   **Task H.3:** Create `IntroState` and `NewCharState` by migrating logic from `Game`. (Status: Complete)
-   **Task H.4:** Create `HomeState` to act as the central hub for the main game. (Status: Complete)
-   **Task H.5:** Refactor `Game`'s modal view management (`_toggle_view`) to be called by the active state. (Status: Complete)
-   **Task H.6:** Refactor `test_game.py` into smaller, state-focused test files. (Status: In Progress)
