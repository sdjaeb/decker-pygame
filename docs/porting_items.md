...
-   **Task H.2.6.E:** Migrate `toggle_home_view` logic from `Game` to `HomeState`. (Status: To Do)
    -   **Task H.2.6.F:** Refactor remaining `toggle_*_view` calls in `Game` to use the `ViewManager`. (Status: To Do)
    -   **Task H.2.6.G:** Remove redundant `toggle_*_view` methods from `Game` once their logic has been fully migrated. (Status: To Do)
