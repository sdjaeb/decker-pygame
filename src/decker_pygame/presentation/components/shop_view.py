...
class ShopView(pygame.sprite.Sprite):
    ...
    Args:
        ...
        on_view_details (Callable[[str], None]): A callback to view item details.
    ...
    def _render_data(self) -> None:
        ...
        for item in self._data.items:
            ...
            # Add "View" button
            buy_button_x = self.image.get_width() - button_size[0] - self._padding
            view_button_x = buy_button_x - button_size[0] - self._padding
            view_button = Button(
                (view_button_x, y_offset),
                button_size,
                "View",
                partial(self._on_view_details, item.name),
            )
            self._components.add(view_button)

            # Add "Buy" button
            buy_button = Button(
                (buy_button_x, y_offset),
                button_size,
                "Buy",
