...
class Game:
    ...
    Attributes:
        ...
        current_state (Optional[BaseState]): The current state of the game.
        view_manager (ViewManager): Manages the creation and destruction of views.
    """
    ...
    current_state: Optional[BaseState] = None
    ...
    def __init__(
        ...
        self.view_manager = ViewManager(self.all_sprites)
    ...
    def set_state(self, new_state: BaseState) -> None:
        """Sets the current game state, handling transitions."""
    ...
    def toggle_matrix_run_view(self) -> None:
        ...
        self.view_manager.toggle_view("matrix_run_view", factory, self)
    ...
    def toggle_project_data_view(self) -> None:
        ...
        self.view_manager.toggle_view("project_data_view", factory, self)
    ...
    def _on_rest(self) -> None:
        ...
        self.view_manager.toggle_view("rest_view", None, self)
    ...
    def toggle_rest_view(self, data: Optional[RestViewDTO] = None) -> None:
        ...
        self.view_manager.toggle_view("rest_view", factory, self)
    ...
    def toggle_mission_results_view(
        ...
        self.view_manager.toggle_view("mission_results_view", factory, self)
    ...
    def toggle_build_view(self) -> None:
        ...
        self.view_manager.toggle_view("build_view", factory, self)
    ...
    def toggle_char_data_view(self) -> None:
        ...
        self.view_manager.toggle_view("char_data_view", factory, self)
    ...
    def toggle_deck_view(self) -> None:
        ...
        self.view_manager.toggle_view("deck_view", factory, self)
    ...
    def toggle_transfer_view(self) -> None:
        ...
        def factory() -> Optional[TransferView]:
            transfer_data = self.deck_service.get_transfer_view_data(self.character_id)
        ...
        self.view_manager.toggle_view("transfer_view", factory, self)

    def _on_order_deck(self) -> None:
        """Callback to handle opening the deck ordering view."""
        char_data = self.character_service.get_character_data(self.character_id)
        if not char_data:
        ...
        # Close the deck view and open the order view
        self.view_manager.toggle_view("deck_view", None, self)
        self.view_manager.toggle_view("order_view", factory, self)
    ...
    def toggle_ice_data_view(self, data: Optional[IceDataViewDTO] = None) -> None:
        ...
        self.view_manager.toggle_view("ice_data_view", factory, self)
    ...
    def toggle_contract_list_view(self) -> None:
        ...
        self.view_manager.toggle_view("contract_list_view", factory, self)
    ...
    def toggle_contract_data_view(self) -> None:
        ...
        self.view_manager.toggle_view("contract_data_view", factory, self)
    ...
    def toggle_file_access_view(self, data: Optional[FileAccessViewDTO] = None) -> None:
        ...
        self.view_manager.toggle_view("file_access_view", factory, self)

    def show_file_access_view(self, node_id: str) -> None:
        """Fetches node data and shows the file access view."""
        if self.view_manager.is_view_open("file_access_view"):
            self.toggle_file_access_view()
            return

        node_files = self.node_service.get_node_files(node_id)
        if node_files:
            self.toggle_file_access_view(node_files)
        else:
            self.show_message(f"Error: Could not access node '{node_id}'.")

    def _on_entry_submit(self, text: str, node_id: str) -> None:
        """Callback for when text is submitted from an entry view."""
        if self.node_service.validate_password(node_id, text):
            self.show_message("Access Granted.")
        else:
            self.show_message("Access Denied.")
        self.view_manager.toggle_view("entry_view", None, self)

    def _on_save_game(self) -> None:
        """Callback for when the 'Save Game' button is clicked."""
        self.show_message("Game Saved (Not Implemented).")

    def _on_load_game(self) -> None:
        """Callback for when the 'Load Game' button is clicked."""
        self.show_message("Game Loaded (Not Implemented).")

    def _on_quit_to_menu(self) -> None:
        """Callback for when the 'Quit to Menu' button is clicked."""
        self.show_message("Quit to Menu (Not Implemented).")
        self.view_manager.toggle_view("options_view", None, self)

    def _on_toggle_sound(self, enabled: bool) -> None:
        """Callback for when the sound toggle is changed."""
    ...
    def _on_toggle_tooltips(self, enabled: bool) -> None:
        """Callback for when the tooltips toggle is changed."""
    ...
    def toggle_options_view(self) -> None:
        ...
        self.view_manager.toggle_view("options_view", factory, self)
    ...
    def toggle_sound_edit_view(self) -> None:
        """Opens or closes the sound editing view."""

        def factory() -> SoundEditView:
        ...
        self.view_manager.toggle_view("sound_edit_view", factory, self)
    ...
    def _on_start_project(self, item_type: str, item_class: str, rating: int) -> None:
        ...
        try:
            ...
            self.view_manager.toggle_view("new_project_view", None, self)
        except Exception as e:
            self.show_message(f"Error: {e}")
    ...
    def _create_new_project_view(
        ...
        """Helper to create an instance of the NewProjectView."""
        if not data:
            self.show_message("Error: Could not retrieve project data.")
            return None
    ...
    def toggle_new_project_view(self) -> None:
        """Opens or closes the new project view."""

        def factory() -> Optional[NewProjectView]:
            # Check if the view is already open to close it
            if self.view_manager.is_view_open("new_project_view"):
                return None
        ...
        self.view_manager.toggle_view("new_project_view", factory, self)

    def toggle_entry_view(self, node_id: Optional[str] = None) -> None:
        """Opens or closes a generic text entry view."""
        ...
        self.view_manager.toggle_view("entry_view", factory, self)
    ...
    def _update(self, dt: int, total_seconds: int) -> None:
        ...
        Args:
            ...
            total_seconds (int): The total time since the game started in seconds.
        """
        if self.current_state:
            self.current_state.update(dt, total_seconds)
    ...
    def run(self) -> None:
        ...
        while self.is_running:
            ...
            total_seconds += dt / 1000.0

            self.input_handler.handle_events()
            self._update(dt, int(total_seconds))

            self.screen.fill(BLACK)
            if self.current_state:
                self.current_state.draw(self.screen)
            self.all_sprites.draw(self.screen)
            self.message_view.draw(self.screen)
            pygame.display.flip()
